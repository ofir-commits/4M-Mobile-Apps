...
s.merge_cells('A6:G6')
w('A6', 'זו בדיקה עצמית מקדימה של קוד התבנית והגדרות החנות, שנועדה לשמש בסיס לבדיקה הרשמית. '
        'היא איננה חוות דעת של מורשה נגישות שירות, ואינה מהווה תחליף לה: תקנה 29 מחייבת שההצהרה '
        'וההתאמות ייערכו בהתייעצות עם מורשה נגישות שירות, והבדיקה התקופתית לפי תקנה 93(א) '
        '(טופס 9) מבוצעת על ידו. את הגיליונות שלהלן יש להעביר למורשה כנקודת פתיחה - הוא יאמת, '
        'ישלים את מה שסומן כטרם נבדק, ויחתום.', wrap=True, size=10)
s.row_dimensions[6].height = 74
...
legend = [
    ...
    ('לא תקין',    'FFC7CE', 'נבדק, ונמצא שאינו עומד בקריטריון. לא נותרו כאלה: ארבעה כשלי ניגודיות, '
                             'כשל בסימון הפוקוס בשדה הכמות וכשל בעצירת התנועה בסרגל ההודעות '
                             'נמצאו במהלך הבדיקה ותוקנו.'),
...
todo = [
    ...
    'חוות דעת של מורשה נגישות שירות, כנדרש בתקנה 29, וחתימתו על טופס זה.',
]
for t in todo:
